...
class AdamBackend(Backend):
    ...
    def evaluate(self, num_iters: int = 1000, start_iter: int = 0,
                 ...
                 timeout: Optional[float] = None) -> Tuple[th.Tensor, float, pd.DataFrame]:
...
class LineSearchBackend(Backend):
    """Gradient-based optimization using line search.
    """
    def evaluate(self, num_iters: int = 100, start_iter: int = 0,
                 save_interval: int = 20, update_interval: int = 10,
                 save_state_callback: SaveStateCallback = lambda *_: ...,
        ...
        """
        del start_iter, save_interval, update_interval, save_state_callback, save_image_callback

        graph, param_io_kwargs, loss_func = self.graph, self.param_io_kwargs, self.loss_func
        _parameters = lambda: graph.parameters(param_io_kwargs)

        # Specify the objective function
        def obj_func(params: npt.NDArray[np.floating]) -> float:
            with th.no_grad():
                loss = self._obj_func(self._t(params), save_interval=0)
            return loss

        # Specify the gradient function
        def grad_func(
                params: Optional[npt.NDArray[np.floating]] = None) -> npt.NDArray[np.floating]:
            # Set parameters
            if params is not None:
                params = self._t(params)
                self._set_parameters(params)
            ...
            loss = loss_func(graph.evaluate())
            loss.backward()

            # Extract the gradients
            grad = th.cat([(p.grad if p.grad is not None else th.zeros_like(p)).flatten()
                           for p in _parameters()])
            return grad.cpu().numpy()

        # Run line search optimization
        cur_params = self._get_parameters().cpu().numpy()
        cur_grad = grad_func()

        alpha, _, _, self.loss_min, _, _ \
            = line_search(obj_func, grad_func, cur_params, -cur_grad, maxiter=num_iters, **kwargs)
        self.param_min = self._t(cur_params - alpha * cur_grad)
...
class AxBO(IntegerBackend):
    ...
    def __init__(self, graph: MaterialGraph, loss_func: BaseMetric, **kwargs):
        ...
        # Check if the `ax` library is installed
        try:
            import ax
        except ImportError:
            raise ImportError('The `Ax` library is required for Bayesian Optimization.')
...
class SkoptBO(IntegerBackend):
    ...
    def __init__(self, graph: MaterialGraph, loss_func: BaseMetric, **kwargs):
        ...
        # Check if the `skopt` library is installed
        try:
            import skopt
        except ImportError:
            raise ImportError('The `scikit-optimize` library is required for Bayesian Optimization.')
...
class SimAnneal(HybridBackend):
    ...
    def __init__(self, graph: MaterialGraph, loss_func: BaseMetric, seed: int = -1,
        ...
        # Check if the `simanneal` library is installed
        try:
            import simanneal
        except ImportError:
            raise ImportError('The `simanneal` library is required for Simulated Annealing.')
...
class GridSearch(HybridBackend):
    ...
    def evaluate(self, num_iters: int = 3, start_iter: int = 0,
                 ...
                 save_state_callback: SaveStateCallback = lambda *_: ...,
                 save_image_callback: SaveImageCallback = lambda *_: ...,
                 search_res: int = 100, timeout: Optional[float] = None) \
                    -> Tuple[Tuple[th.Tensor, List[int]], float, Any]:
        """Run grid search.
        """
...
BACKEND_DICT: Dict[str, Dict[str, Type[BaseBackend]]] = {
    'continuous': {'adam': AdamBackend, 'line': LineSearchBackend},
...
try:
    ...
    class SimAnnealWorker:
        """Dummy class for Simulated Annealing worker.
        """
